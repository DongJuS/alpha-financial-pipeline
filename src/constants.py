...
# ── Screener (일봉 기반 종목 필터링) ──────────────────────────────────
MAX_SCREENED_TICKERS: int = 10  # 스크리너 통과 후 전략 실행 최대 종목 수
DEFAULT_COLLECTOR_DAILY_LIMIT: int = 100  # 일봉 수집 기본 종목 수 (시가총액 상위)
SCREENER_VOLUME_SURGE_RATIO: float = 2.0  # 거래량 급등 기준 (20일 평균 대비 배수)
SCREENER_CHANGE_PCT_THRESHOLD: float = 3.0  # 가격 변동률 기준 (±%)
