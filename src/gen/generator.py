...
class MarketDataGenerator:
    ...
    def __init__(
        self,
        volatility: float = 0.02,      # 1틱 변동률 표준편차 (2%)
        drift: float = 0.0001,          # 미세 상승 drift
    ...
    def generate_tick(self) -> list[GenTick]:
        ...
        with self._lock:
            ...
            for t in self._tickers:
                # GBM: S(t+1) = S(t) * exp((drift - 0.5*vol^2) + vol*Z)
    ...
    def generate_daily_history(self, ticker: str, days: int = 120) -> list[GenOHLCV]:
        ...
        with self._lock:
            ...
            # 역방향으로 가격 생성 후 순방향으로 뒤집음
            prices_rev: list[float] = [price]
            for _ in range(days - 1):
                z = self._rng.gauss(0, 1)
                daily_vol = 0.015  # 일간 변동률
                log_return = self._rng.gauss(0, daily_vol)
