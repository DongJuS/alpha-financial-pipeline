...
async def _run_cycle_if_weekday(
    ...
    *,
    ...
    return await agent.run_cycle(
        tickers=resolved, screener_kwargs=screener_kwargs, strategies=strategies,
    )
...
async def main_async() -> int:
    ...
    try:
        while True:
            ...
            schedules: list[dict] = []
            try:
                from src.db.queries import fetch_prediction_schedules, touch_prediction_schedule
                schedules = await fetch_prediction_schedules()
            except Exception as e:
                logger.debug("prediction_schedule 조회 실패 (fallback 사용): %s", e)
            ...
            if schedules:
                ...
                for sched in schedules:
                    ...
                    interval = timedelta(minutes=sched["interval_minutes"])
                    if last_run is None:
                        strategies_to_run.append(sched["strategy_code"])
                    else:
                        # last_run_at이 tz-naive일 수 있으므로 KST로 보정
                        if last_run.tzinfo is None:
                            last_run = last_run.replace(tzinfo=_KST)
                        if (now_kst - last_run) >= interval:
                            strategies_to_run.append(sched["strategy_code"])

            # fallback: DB 스케줄 없으면 기존 환경변수 간격으로 전체 실행
            use_legacy_interval = not schedules
            if use_legacy_interval:
                # 레거시 모드: interval_seconds 간격으로 전체 전략 실행
                strategies_to_run = None  # None = 모든 전략 실행

            if strategies_to_run is None or strategies_to_run:
                if strategies_to_run is not None:
                    logger.info("스케줄 기반 전략 실행: %s", strategies_to_run)
                try:
                    await _run_cycle_if_weekday(
                        agent, tickers,
                        screener_kwargs=screener_kwargs,
                        strategies=strategies_to_run,
                    )
                    # 실행한 전략의 last_run_at 갱신
                    if strategies_to_run and schedules:
                        for code in strategies_to_run:
                            try:
                                await touch_prediction_schedule(code)
                            except Exception:
                                pass
                except Exception as e:
            ...
            # DB 스케줄 모드: 1분 간격 체크 / 레거시 모드: 기존 interval_seconds
            sleep_seconds = 60 if not use_legacy_interval else interval_seconds
