...
| 날짜 | 작업 내용 | 상태 |
|------|-----------|------|
| 2026-03-15 | **Search Strategy (S) 통합 — ResearchPortfolioManager → SearchRunner 어댑터** — `src/agents/orchestrator.py`에 `SearchRunner` 클래스 추가(name="S", `run()` 메서드), `OrchestratorAgent.__init__()`에 `use_search` 플래그/search 파라미터 추가, `_resolve_strategies()` 업데이트(--search 플래그 지원), `_setup_registry()` 업데이트(SearchRunner 등록), CLI에 `--search`, `--search-max-concurrent`, `--search-categories`, `--search-max-sources` 추가, `src/db/models.py` `PaperOrderRequest.signal_source`에 "S" 추가, `src/utils/config.py` strategy_blend_weights 기본값 변경(`{"A": 0.30, "B": 0.30, "RL": 0.20, "S": 0.20}`), SEARCH_* 설정 추가, `.env.example` 업데이트, `test/test_search_runner.py` 포괄적 테스트 추가(sentiment→signal 매핑, 캐싱, 에러 핸들링, 프로토콜 준수), `docs/AGENTS.md`에 ResearchPortfolioManager 섹션 추가, `architecture.md` N-way 다이어그램 업데이트, `MEMORY.md` 기술적 결정 기록 | ✅ 완료 |
| 2026-03-15 | **KOSPI/KOSDAQ 지수 수집 파이프라인 구현 (C-1 해소)** — `src/brokers/kis.py`에 `fetch_index_quote()` 메서드 추가(KIS API `/inquire-index-price`, TR_ID `FHPUP02100000`, KOSPI=0001/KOSDAQ=1001), `src/agents/index_collector.py` 신규(IndexCollector, Redis 캐시 120초 TTL), `src/schedulers/index_scheduler.py` 신규(APScheduler AsyncIOScheduler, 30초 간격 수집 + 08:55 워밍업, 장중만 실행), `src/utils/redis_client.py`에 KEY_MARKET_INDEX/TTL_MARKET_INDEX 상수 추가, `src/api/main.py` lifespan에 스케줄러 시작/종료 연동 | ✅ 완료 |
| 2026-03-15 | **QA M-2/M-3/M-4 근본 원인 수정 + 시스템 플로우차트** — [M-2] `predictor.py`에 보유 포지션 컨텍스트(수량, 평균단가, 평가손익률) 전달 + `portfolio_manager.py`에 규칙 기반 익절(+5%)/손절(-3%) 자동 매도 로직 추가 + `queries.py`에 `get_positions_for_scope()` 추가, [M-3] 예측기별 temperature 분산(0.3~0.7) 적용, Gemini/GPT `ask_json()`에 `_extract_json` 마크다운 JSON 추출 로직 추가, [M-4] Debate 역할별 temperature 설정(Proposer 0.5/Challenger 0.7/Synthesizer 0.3), fallback 로깅 강화, `docs/system_flowchart.mermaid` 전체 시스템 플로우차트 작성 | ✅ 완료 |
| 2026-03-15 | **QA 보고서 기반 프론트엔드/백엔드 버그 수정** — [C-2] `strategy.py` `/a/signals` SQL 파라미터 조건 반전 버그 수정(date=None일 때 $1 사용하면서 파라미터 미전달 → CURRENT_DATE 직접 사용), [C-3] `db_client.py` asyncpg JSONB 자동 디코딩 코덱 추가(`_init_connection`으로 json/jsonb set_type_codec 설정 → `/readiness/audits` 500 에러 해소), [M-1] 대시보드 "총 손익"→"미실현 손익" 라벨 명확화, 포트폴리오 "수익률"→"실현 수익률(매도 완료 기준)" 라벨 변경, [m-1] Market 실시간 차트 장마감 시 "장 마감 — 마지막 체결가 기준" 안내 표시, [m-2] confidence/Sharpe toFixed(3)→toFixed(2) 소수점 2자리 통일 (Strategy.tsx, Portfolio.tsx, PaperTrading.tsx), [m-3] MDD formatPct→formatMDD 전환(0일 때 "0.00%", 음수일 때 "-X.XX%"), [m-4] 포트폴리오 승률 매도 없을 때 "—"/"매도 없음" 표시 | ✅ 완료 |
...
| 2026-03-15 | **RL 독립 트레이딩 실행** — `src/agents/orchestrator.py`에서 RL을 N-way 블렌딩에서 분리, `--strategies A,B,RL` 실행 시 A+B만 블렌딩하고 RL은 독립적으로 PortfolioManager에 직접 전달, `blend_meta`에 `rl_independent`/`rl_predictions`/`rl_orders` 추가, `test/test_blend_nway.py`에 `TestRLIndependentExecution` 4개 테스트 추가 | ✅ 완료 |
