...
- [x] **QA 잔여 이슈 (C3, H1~H4, M1~M4)** — 코드 리뷰 체크리스트 전체 처리 완료 (PR #28). gen/KIS 스키마 정합성 검증 포함 (32 tests all pass)
...
- ~~**스케줄러가 IndexCollector만 가동**~~ → 2026-03-29 해결: 통합 스케줄러(`unified_scheduler.py`)로 4개 잡(stock_master/macro/collector/index) 단일 관리. 분산 락 + 재시도 + 실행 이력 추가
- **RLRunner 활성 정책 없으면 0건 반환** — 학습→정책 활성화 파이프라인 미실행 시 Strategy RL은 항상 빈 시그널. 운영 시작 전 활성 정책 등록 필요
- ~~**gen 모드 주말 스킵 우회 미구현**~~ → 2026-03-29 해결: `GEN_API_URL` 설정 시 `_run_cycle_if_weekday` 및 RL retrain 스케줄러에서 주말 체크 우회

---

*Last updated: 2026-03-29 (gen/KIS 스키마 정합성 검증 완료)*
