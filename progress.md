...
## ✅ 최근 완료 (2026-03-31 ~ 04-08)

### Secret 단일 소스화 (SOPS+age) — PR #104
- `k8s/base/secrets.yaml`(평문 placeholder) 삭제, `k8s/secrets/app-secret.enc.yaml` 로 통합
- `deploy-local.sh` 가 `sops --decrypt | kubectl apply` 단계를 자동 수행
- 운영 가이드 `docs/secrets.md`, 부트스트랩 `secrets-bootstrap.sh`, 단위 테스트 10건 (test/test_secrets_sops.sh)
- postgres 비번 drift 사고(#102 deploy 차단) 재발 방지

### Gen 데이터 격리 (alpha_gen_db) — PR #105
- gen_collector 가 실 alpha_db 의 `market_data` 를 오염시키던 문제 (gen 이
  주말/장외에 항상 돌면서 dead-write 누적) 를 별도 DB 로 격리
- `src/agents/gen_collector.py` 가 import 시점에 DATABASE_URL 의 database
  부분을 `alpha_gen_db` 로 자동 rewrite (`GEN_DB_REWRITE_DISABLED=1` 로 끌 수 있음)
- `scripts/db/init_db.py` 가 기본으로 alpha_db + alpha_gen_db 둘 다 생성/초기화
  (`--skip-gen-db` 로 옵트아웃, `_swap_database_name` + `_ensure_database_exists` 헬퍼 추가)
- docker-compose.yml / docker-compose.prod.yml: gen / gen-collector 를
  `profiles: [gen]` 로 옵트인 — 기본 `docker compose up` 에서는 안 뜸.
  `docker compose --profile gen up` 으로 명시 기동.
- k8s: `k8s/base/gen.yaml` → `k8s/overlays/gen/` 로 분리. base 에서 제거.
  bitnami postgres-values 에 `initdb.scripts` 로 alpha_gen_db 자동 생성 추가.
- 회귀 가드 단위 테스트 5건 (rewrite / no-op / disabled / custom-name + swap helper)
- isolated postgres 컨테이너로 격리 실측 검증:
  alpha_db 카나리 row(999999) 보존 + gen 데이터(005930)는 alpha_gen_db 에만 적재

### RL split adaptive (ε-greedy bandit) — PR #TBD
- `(ticker, profile_id)`별 ε-greedy bandit으로 train/test split 비율을 사이클마다 동적 선택
- `artifacts/rl/bandit/{ticker}__{profile_id}.json` 파일 기반(atomic write) 영속화
- profile JSON에 `adaptive_ratios=[0.5,0.6,0.7,0.8]` + `bandit_epsilon=0.2` 추가, `default_train_ratio`는 fallback로 보존
- `/feedback/rl/retrain/*` 응답에 `selected_train_ratio` + `bandit_snapshot` 노출
- 신규 단위 테스트 3종 (split_bandit 22 + improver_split 6 + feedback_router_rl 6) + train_ratio 검증 4건
- RL 관련 118 테스트 전체 통과
