...
Step 10          백테스트 시뮬레이션       █████████░   90% 🔧 (E2E 1건 실패: SELL 미생성)
안정화 스프린트   Step 10 검증 + CI 정리   █████████░   90% 🔧 ← 현재
...
Step 10 백테스트 4개 PR(#111~#114) 병렬 머지 후 통합 검증. PR #115~#117로 P0~P1 대부분 완료.

### 완료 (PR #115~#119)

| # | 항목 | PR | 상태 |
|---|------|-----|------|
| 1 | integration test CI 분리 | `pytest.ini --ignore=test/integration` | [x] |
| 2 | `gymnasium` 의존성 추가 + 미커밋 정리 + backtest DDL 확인 | #116 | [x] |
| 3 | backtest CLI 기동 + 4-PR 인터페이스 불일치 수정 | #115 | [x] |
| 4 | engine ↔ signal_source ↔ cost_model 통합 테스트 2개 | #117 | [x] |
| 5 | `fetch_ohlcv_range()` 추가 + RL position state mismatch fix | #118 | [x] |
| 6 | cli.py `_load_ohlcv()` → `fetch_ohlcv_range` 전환 + fixture CSV | #119 | [x] |
| 7 | 테스트: 735 passed (612 → 735) | — | [x] |
| 8 | E2E test 작성 | `test/test_backtest_e2e_rl.py` | [x] Agent 3 |

### E2E 테스트 현황

`test/test_backtest_e2e_rl.py` 작성 완료. **1건 실패**: `test_buy_sell_mixed` — SELL 시그널 미생성 (BUY만 발생).
원인: Q-table 학습 결과가 fixture 데이터에서 SELL을 선택하지 않는 것으로 추정. seed/학습 파라미터 조정 또는 assert 조건 완화 필요.

### 잔여

| # | 항목 | 상태 |
|---|------|------|
| 8 | Docker Compose 전체 기동 + smoke test | [ ] |
| 9 | gen 격리(alpha_gen_db) 주말 사이클 검증 | [ ] |
...
### Step 10: 백테스트 시뮬레이션 (코드+안정화 완료, 실데이터 E2E 착수 예정)
**PR #111~#114 코드 + PR #115~#119 안정화.** CLI 기동·인터페이스 수정·통합 테스트·데이터 로딩 전환 완료.
