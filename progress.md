...
- [ ] **approval_key Redis 캐싱** — 24h 만료 키를 TTL 22h로 캐싱, `_ensure_ws_approval_key()` 패턴 적용
...
- [ ] `redis_client.py` — set_heartbeat() Hash 확장 (status/mode/last_data_at/error_count)
- [ ] `collector.py` — _beat()에서 WS 상태 반영 (ok/degraded/error, websocket/fdr/idle)
- [ ] `scripts/docker_healthcheck.py` — 신규 (Redis heartbeat 키+status 체크)
- [ ] `docker-compose.yml` — worker healthcheck → docker_healthcheck.py
- [ ] `scripts/health_check.py` — heartbeat status 체크 추가
- [ ] K8s probe — livenessProbe(키 존재) / readinessProbe(status!=error) 분리
- [ ] `docs/HEARTBEAT.md` — 실제 구현과 일치하도록 업데이트
- [ ] 단위 테스트 (heartbeat Hash, docker_healthcheck, collector status 전환)
