...
def _mock_policy_store():
    """RLPolicyStoreV2 mock."""
    store = MagicMock()
    registry = MagicMock()
    registry.tickers = {}
    registry.promotion_gate = MagicMock()
    registry.promotion_gate.auto_promote_paper_only = True
    store.load_registry.return_value = registry
    return store
...
class TestPolicyMode:
    ...
    def test_inactive_policy_no_records(self):
        engine = _engine()
        mode = engine.get_policy_mode("unknown_policy", "005930")
        assert mode == "inactive"

    def test_shadow_mode_with_records(self):
        engine = _engine()
        engine.create_shadow_signal(
            policy_id="p1", ticker="005930", signal="BUY",
            confidence=0.7, close_price=100.0,
