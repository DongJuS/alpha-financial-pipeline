...
async def test_buy_at_or_above_threshold_proceeds():
    ...
    mock_resolve = AsyncMock(return_value=("삼성전자", 0))
    with patch.object(agent, "_resolve_name_and_price", new=mock_resolve):
        result = await agent.process_signal(signal, risk_config=CFG_GATE_ON)
        # gate 통과. price=0 로 인해 skip 됐지만 gate 판정은 성공.
        assert result is None
...
async def test_rule_based_exit_bypasses_gate():
    ...
    with patch.object(agent, "_resolve_name_and_price", new=mock_resolve):
        result = await agent.process_signal(signal, risk_config=CFG_GATE_ON)
        # gate 미적용 (trigger_source 있음). price=0 이후 skip 이지만 gate
        # 판정이 skip 아님. _resolve_name_and_price 호출로 확인.
        assert result is None
        mock_resolve.assert_awaited_once()
...
async def test_gate_disabled_ignores_low_confidence():
    ...
    with patch.object(agent, "_resolve_name_and_price", new=mock_resolve):
        ...
        mock_resolve.assert_awaited_once()  # gate 뚫음
