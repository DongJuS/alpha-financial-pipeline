...
export interface DataLakeOverview {
  ...
  prefixes: Array<{ prefix: string; count: number; size: number; size_display: string }>;
}
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<S3ObjectListResponse> => {
      const { data } = await api.get<S3ObjectListResponse>("/datalake/objects", {
        params: { prefix, limit: 100, delimiter: "/" },
      });
      return data;
    },
    staleTime: 30_000,
...
export function useDeleteDatalakeObject() {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["datalake"] });
    },
  });
}
