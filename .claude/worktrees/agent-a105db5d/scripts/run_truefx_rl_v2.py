"""
scripts/run_truefx_rl_v2.py — TrueFX CSV 기반 EUR/USD RL V2 학습/테스트

V1 대비 변경:
- TabularQTrainerV2 사용
- 기본 에피소드 100 (FX 틱 데이터에 맞게 조정, V1은 8이었음)
- 집계 단위: 60초 기본 (V1 1초 → 노이즈 과다)
- 멀티시드 3회

기본값:
- 입력: artifacts/rl/data/EURUSD-*.csv
- 전처리: bid/ask -> 60초 단위 mid-price close
- 분할: 80% 학습 / 20% 테스트
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TrueFX CSV 기반 RL V2 학습/테스트")
    parser.add_argument(
        "--pattern",
    ...
    parser.add_argument("--aggregate-seconds", type=int, default=60, help="집계 초 단위 (V1: 1, V2: 60)")
    parser.add_argument("--episodes", type=int, default=100, help="Q-learning episodes (V1: 8)")
    ...
    parser.add_argument("--num-seeds", type=int, default=3, help="multi-seed training runs")
    ...
    result_path = output_dir / f"{symbol}_latest_run_v2.json"
